...
- Changelog 1.5.0: closed 20 spec gaps from `webdevskills-engine-completion-2026` Depth-2 specs. Created 3 new skills — `postgresql-patterns` (Postgres-as-second-DB: JSONB, FTS, pgvector, RLS, PgBouncer, MySQL→PG translations), `vector-databases` (engine selection across pgvector/Qdrant/Pinecone/Weaviate, embeddings, chunking, hybrid search, reranking), `rag-implementation` (Naive→Advanced→Modular progression, query transforms, corrective RAG, RAGAS evaluation, multi-tenant isolation). Reconciled 10 Bucket A skills (`cloud-architecture`, `kubernetes-platform`, `infrastructure-as-code`, `cicd-pipelines`, `observability-platform`, `stripe-payments`, `subscription-billing`, `android-ai-ml`, `e2e-testing`, `pwa-offline-first`) against their content extracts. Enhanced 7 existing skills with new sections — `cicd-devsecops` (Vault cluster + PKI lifecycle, ISO 27001 Annex A, PCI-DSS scope reduction, Falco/Gatekeeper/Trivy), `database-reliability` (SLO/SLI, error budgets, blameless postmortems, escalation, game days), `microservices-architecture-models` (HAProxy/Kong/Traefik ops + decision matrix), `web-app-security-audit` (network-layer audit), `cicd-pipeline-design` (FinOps Foundation framework), `cicd-jenkins-debian` (Linux hardening + perf tuning), `microservices-communication` (n8n/Temporal/Airflow with retry-policy table). Added ~40 new `references/` files. Repository now at 248 skills, all touched skills validate clean.
