...
description: Compose Multiplatform for shared UI across Android, iOS, Desktop (JVM), and Web (wasmJs). Covers targets configuration, shared composables, desktop windowing, wasmJs browser targets, navigation, platform-specific UI hooks, CI/CD for multi-target builds, and performance optimisation. Companion to kmp-development (business logic) and android-development (Android-only UI).
---
