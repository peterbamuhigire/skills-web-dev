---
name: ai-agent-customer-sla-dashboard
description: Use when designing the per-tenant customer-facing dashboard that exposes agent SLA metrics — tasks completed, tasks attempted, resolution rate, time-to-resolve p50/p95, intervention rate, irreversible-incident counter, SLA-credit balance, SLA-tier — plus an embeddable widget and a public API. The same numbers used to issue credits are the numbers the customer sees. Distinct from internal ops dashboards.
metadata:
  portable: true
  compatible_with:
  - claude-code
