---
name: ai-agent-evidence-automation
description: Use when building the automated evidence collection pipelines for compliance — cron / triggered exporters, evidence catalogue, evidence-pack format, tie-in to incident evidence capture, and the read-only auditor portal. The mechanics that turn per-control collectors into auditor-ready packs and serves them on demand.
metadata:
  portable: true
  compatible_with:
  - claude-code
