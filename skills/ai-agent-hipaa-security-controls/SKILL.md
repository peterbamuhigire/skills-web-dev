---
name: ai-agent-hipaa-security-controls
description: Use when implementing the HIPAA Security Rule (45 CFR §§ 164.306–.318) for agents handling Protected Health Information (PHI) — administrative, physical, and technical safeguards; admin-only constraint for clinical PHI agents; BAA (Business Associate Agreement) decision tree for LLM providers in the loop; access controls, audit controls, integrity, transmission security, contingency planning. Engineering side; pair with the SRS engine for the HIPAA Risk Analysis and Risk Management Plan.
metadata:
  portable: true
  compatible_with:
  - claude-code
