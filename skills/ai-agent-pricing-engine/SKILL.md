---
name: ai-agent-pricing-engine
description: Use when engineering the pricing engine for agent products — per-resolution / per-outcome / per-step / per-agent / hybrid pricing. Deterministic resolver from usage event to invoice line item amount, with intervention-credit reduction, vendor-cost-pass-through with markup, abort-and-refund logic, FX corridor, volume tiers. Distinct from `software-pricing-strategy` (commercial decision) and `ai-usage-metering-and-billing` (token credit metering) by being a price-rule resolver service.
metadata:
  portable: true
  compatible_with:
  - claude-code
