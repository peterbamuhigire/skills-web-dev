---
name: ai-agent-revenue-recognition
description: Use when engineering ASC 606 / IFRS 15 revenue recognition specifically for agent-product revenue — point-in-time at resolution, deferred revenue for prepaid task credits, refund reserves, intervention-adjusted booking, and the month-end close pipeline. Distinct from generic `saas-accounting-system` (subscription revenue) by being agent-shaped (per-resolution, per-attempt, per-intervention class). Coordinated with `subscription-billing` and `ai-usage-metering-and-billing`.
metadata:
  portable: true
  compatible_with:
  - claude-code
