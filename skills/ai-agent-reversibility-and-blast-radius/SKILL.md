---
name: ai-agent-reversibility-and-blast-radius
description: Use when engineering reversibility into agent actions and capping their blast radius — dry-run modes, staged commits, transactional groups (saga + compensating actions), per-tenant / per-role / per-session caps, sandbox-first execution. Distinct from `ai-agent-tool-catalogue-and-action-gating` (which *classifies* reversibility) by being the *engineering patterns* that make actions reversible and bound damage.
metadata:
  portable: true
  compatible_with:
  - claude-code
