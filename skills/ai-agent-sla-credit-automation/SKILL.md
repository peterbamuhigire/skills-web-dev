---
name: ai-agent-sla-credit-automation
description: Use when engineering the automated pipeline that detects an SLA breach, checks eligibility, calculates the credit, issues a Stripe credit-note (or analog), notifies the customer, and writes an audit row. Idempotent, evidence-pack-backed, and integrated with `ai-agent-sla-and-commitments` (defines commitments) and `ai-agent-task-success-tracking` (produces the verdicts). Distinct from manual finance refunds by being machine-driven and customer-trust-positive.
metadata:
  portable: true
  compatible_with:
  - claude-code
