---
name: ai-agent-task-success-tracking
description: Use when engineering the production pipeline that decides whether each live agent task actually succeeded — heuristic → LLM-judge → human-verification cascade, per-feature success contracts, dispute resolution, per-tenant success-rate dashboards and trending. The verdicts produced here feed billing (`ai-agent-attempted-vs-completed-billing`) and SLA enforcement (`ai-agent-sla-credit-automation`). Distinct from `ai-agent-eval` (golden-set eval) by running on every production task.
metadata:
  portable: true
  compatible_with:
  - claude-code
