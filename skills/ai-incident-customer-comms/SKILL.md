---
name: ai-incident-customer-comms
description: Use when designing or executing customer and regulator communications during an AI incident — status-page templates per failure class, per-tenant named notifications, per-severity escalation cadence, EU AI Act Article 73 serious-incident reporting, GDPR breach windows, sector regulator notifications, and the language patterns that name what happened without making the situation worse.
metadata:
  portable: true
  compatible_with:
  - claude-code
