---
name: ai-incident-recovery-and-rollback
description: Use when planning and executing recovery from an AI incident — the rollback patterns (prompt pin, model pin, retrieval index pin, tool version pin, gateway routing pin, full feature rollback), the safe re-promotion pathway (shadow-mode comeback, canary-during-recovery, eval-gated re-promotion), and the "do not re-promote until X" gates that ensure the same incident doesn't recur tomorrow.
metadata:
  portable: true
  compatible_with:
  - claude-code
